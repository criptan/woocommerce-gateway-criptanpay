...
class WC_Criptanpay_API {
	 ...
	 */
	const ENDPOINT           = 'https://api.criptan.es/business/charge';
	 ...
	 * Criptanpay can identify our application.
	 *
	 * @since 4.0.0
	 * @version 4.0.0
	 */
	public static function get_user_agent() {
		$app_info = array(
			'name'    => 'WooCommerce Criptanpay Gateway',
			'version' => WC_CRIPTANPAY_VERSION,
			'url'     => 'https://woocommerce.com/products/criptanpay/',
		);
	...
	public static function get_headers() {
		...
		return apply_filters(
			...
			// 	'X-Criptanpay-Client-User-Agent' => json_encode( $user_agent ),
			// )
			array(
				'x-api-key' => base64_encode( self::get_secret_key() . ':' ),
				'Criptanpay-Version'             => self::CRIPTANPAY_API_VERSION
			)
		);
	}
	...
	public static function request( $request, $api = 'charges', $method = 'POST', $with_headers = false ) {
		WC_Criptanpay_Logger::log( "{$api} request: " . print_r( $request, true ) );
		...
		if ( 'charges' === $api && 'POST' === $method ) {
			...
			$idempotency_key = apply_filters( 'wc_criptanpay_idempotency_key', $request['metadata']['order_id'] . '-' . $source, $request );
		...
		$response = wp_safe_remote_post(
			...
			array(
				'method'  => $method,
				'headers' => $headers,
				'body'    => apply_filters( 'woocommerce_criptanpay_request_body', $request, $api ),
				'timeout' => 70,
			)
		);
	...
	/**
	 * Send the request to Criptanpay's API with level 3 data generated
	...
	public static function request_with_level3_data( $request, $api, $level3_data, $order ) {
		...
		// Criptanpay in the past for this account, do not try to add level3 data.
		if ( get_transient( 'wc_criptanpay_level3_not_allowed' ) ) {
			return self::request(
				$request,
				$api
			);
		...
		if ( $is_level3_param_not_allowed ) {
			...
			set_transient( 'wc_criptanpay_level3_not_allowed', true, 3 * MONTH_IN_SECONDS );
		...
		if ( $is_level3_param_not_allowed || $is_level_3data_incorrect ) {
			...
			return WC_Criptanpay_API::request(
				$request,
				$api
			);
		}
