 ...
 * @since 4.0.0
 */
class WC_Criptanpay_Webhook_Handler extends WC_Criptanpay_Payment_Gateway {
	/**
	 * Delay of retries.
	 *
	 * @var int
	...
	public function is_valid_request( $request_headers = null, $request_body = null ) {
		...
		// if ( ! empty( $request_headers['USER-AGENT'] ) && ! preg_match( '/Criptanpay/', $request_headers['USER-AGENT'] ) ) {
		// 	return false;
		// }
	...
	public function process_webhook_payment( $notification, $retry = true ) {
		...
		$order = WC_Criptanpay_Helper::get_order_by_source_id( $notification->data->object->id );

		if ( ! $order ) {
			WC_Criptanpay_Logger::log( 'Could not find order via source ID: ' . $notification->data->object->id );
			return;
		}
		...
		try {
			...
			WC_Criptanpay_Logger::log( "Info: (Webhook) Begin processing payment for order $order_id for the amount of {$order->get_total()}" );
			...
			do_action( 'wc_gateway_criptanpay_process_webhook_payment', $response, $order );
			...
			do_action( 'wc_gateway_criptanpay_process_webhook_payment_error', $order, $notification, $e );
	...
	public function process_webhook_dispute( $notification ) {
		$order = WC_Criptanpay_Helper::get_order_by_charge_id( $notification->data->object->charge );

		if ( ! $order ) {
			WC_Criptanpay_Logger::log( 'Could not find order via charge ID: ' . $notification->data->object->charge );
			return;
		}

		/* translators: 1) The URL to the order. */
		$order->update_status( 'on-hold', sprintf( __( 'A dispute was created for this order. Response is needed. Please go to your <a href="%s" title="Criptanpay Dashboard" target="_blank">Criptanpay Dashboard</a> to review this dispute.', 'woocommerce-gateway-criptanpay' ), $this->get_transaction_url( $order ) ) );

		do_action( 'wc_gateway_criptanpay_process_webhook_payment_error', $order, $notification );
	 ...
	 * transaction that is later captured via Criptanpay not WC.
	 *
	 * @since 4.0.0
	 * @version 4.0.0
	 * @param object $notification
	...
	public function process_webhook_capture( $notification ) {
		$order = WC_Criptanpay_Helper::get_order_by_charge_id( $notification->data->object->id );

		if ( ! $order ) {
			WC_Criptanpay_Logger::log( 'Could not find order via charge ID: ' . $notification->data->object->id );
			return;
		}
		...
		if ( 'criptanpay' === $order->get_payment_method() ) {
			...
			if ( $charge && 'no' === $captured ) {
				...
				if ( $this->is_partial_capture( $notification ) ) {
					...
					$order->add_order_note( sprintf( __( 'This charge was partially captured via Criptanpay Dashboard in the amount of: %s', 'woocommerce-gateway-criptanpay' ), $partial_amount ) );
				} else {
					$order->payment_complete( $notification->data->object->id );

					/* translators: transaction id */
					$order->add_order_note( sprintf( __( 'Criptanpay charge complete (Charge ID: %s)', 'woocommerce-gateway-criptanpay' ), $notification->data->object->id ) );
				}
	...
	public function process_webhook_charge_failed( $notification ) {
		...
		$order->update_status( 'failed', __( 'This payment failed to clear.', 'woocommerce-gateway-criptanpay' ) );
	 ...
	 *
	 * @since 4.0.0
	 * @since 4.1.15 Add check to make sure order is processed by Criptanpay.
	 * @param object $notification
	 */
	public function process_webhook_source_canceled( $notification ) {
		...
		if ( ! $order ) {
			...
			if ( ! $order ) {
				WC_Criptanpay_Logger::log( 'Could not find order via charge/source ID: ' . $notification->data->object->id );
				return;
			}
		}

		// Don't proceed if payment method isn't Criptanpay.
		if ( 'criptanpay' !== $order->get_payment_method() ) {
			WC_Criptanpay_Logger::log( 'Canceled webhook abort: Order was not processed by Criptanpay: ' . $order->get_id() );
			return;
		}

		if ( ! $order->has_status( 'cancelled' ) ) {
			$order->update_status( 'cancelled', __( 'This payment has cancelled.', 'woocommerce-gateway-criptanpay' ) );
		}
	...
	public function process_webhook_refund( $notification ) {
		...
		if ( 'criptanpay' === $order->get_payment_method() ) {
			$charge    = $order->get_transaction_id();
			$captured  = $order->get_meta( '_criptanpay_charge_captured', true );
			$refund_id = $order->get_meta( '_criptanpay_refund_id', true );
			...
			if ( $charge ) {
				$reason = ( isset( $captured ) && 'yes' === $captured ) ? __( 'Refunded via Criptanpay Dashboard', 'woocommerce-gateway-criptanpay' ) : __( 'Pre-Authorization Released via Criptanpay Dashboard', 'woocommerce-gateway-criptanpay' );
				...
				if ( is_wp_error( $refund ) ) {
					WC_Criptanpay_Logger::log( $refund->get_error_message() );
				}

				$order->update_meta_data( '_criptanpay_refund_id', $notification->data->object->refunds->data[0]->id );
				...
				$refund_message = ( isset( $captured ) && 'yes' === $captured ) ? sprintf( __( 'Refunded %1$s - Refund ID: %2$s - %3$s', 'woocommerce-gateway-criptanpay' ), $amount, $notification->data->object->refunds->data[0]->id, $reason ) : __( 'Pre-Authorization Released via Criptanpay Dashboard', 'woocommerce-gateway-criptanpay' );
	...
	public function get_refund_amount( $notification ) {
		if ( $this->is_partial_capture( $notification ) ) {
			...
			if ( in_array( strtolower( $notification->data->object->currency ), WC_Criptanpay_Helper::no_decimal_currencies() ) ) {
