...
/**
 * WC_Criptanpay_Intent_Controller class.
 *
 * Handles in-checkout AJAX calls, related to Payment Intents.
 */
class WC_Criptanpay_Intent_Controller {
	/**
	 * Holds an instance of the gateway class.
	 *
	 * @since 4.2.0
	 * @var WC_Gateway_Criptanpay
	 */
	protected $gateway;
	...
	public function __construct() {
		add_action( 'wc_ajax_wc_criptanpay_verify_intent', array( $this, 'verify_intent' ) );
		add_action( 'wc_ajax_wc_criptanpay_create_setup_intent', array( $this, 'create_setup_intent' ) );
	}
	 ...
	 *
	 * @since 4.2.0
	 * @return WC_Gateway_Criptanpay
	 */
	protected function get_gateway() {
		if ( ! isset( $this->gateway ) ) {
			if ( class_exists( 'WC_Subscriptions_Order' ) && function_exists( 'wcs_create_renewal_order' ) ) {
				$class_name = 'WC_Criptanpay_Subs_Compat';
			} else {
				$class_name = 'WC_Gateway_Criptanpay';
			}
	 ...
	 *
	 * @since 4.2.0
	 * @throws WC_Criptanpay_Exception An exception if there is no order ID or the order does not exist.
	 * @return WC_Order
	 */
	protected function get_order_from_request() {
		if ( ! isset( $_GET['nonce'] ) || ! wp_verify_nonce( sanitize_key( $_GET['nonce'] ), 'wc_criptanpay_confirm_pi' ) ) {
			throw new WC_Criptanpay_Exception( 'missing-nonce', __( 'CSRF verification failed.', 'woocommerce-gateway-criptanpay' ) );
		}
		...
		if ( ! $order ) {
			throw new WC_Criptanpay_Exception( 'missing-order', __( 'Missing order ID for payment confirmation', 'woocommerce-gateway-criptanpay' ) );
		}
	...
	public function verify_intent() {
		...
		try {
			...
			$message = sprintf( __( 'Payment verification error: %s', 'woocommerce-gateway-criptanpay' ), $e->getLocalizedMessage() );
	 ...
	 *
	 * @since 4.2.0
	 * @param WC_Criptanpay_Exception $e           The exception that was thrown.
	...
	protected function handle_error( $e, $redirect_url ) {
		...
		WC_Criptanpay_Logger::log( $message );
	...
	public function create_setup_intent() {
		if (
			! is_user_logged_in()
			|| ! isset( $_POST['criptanpay_source_id'] )
			|| ! isset( $_POST['nonce'] )
		) {
			return;
		}
		...
		try {
			$source_id = wc_clean( $_POST['criptanpay_source_id'] );

			// 1. Verify.
			if (
				! wp_verify_nonce( sanitize_key( $_POST['nonce'] ), 'wc_criptanpay_create_si' )
				|| ! preg_match( '/^src_.*$/', $source_id )
			) {
				throw new Exception( __( 'Unable to verify your request. Please reload the page and try again.', 'woocommerce-gateway-criptanpay' ) );
			}


			// 2. Load the customer ID (and create a customer eventually).
			$customer = new WC_Criptanpay_Customer( wp_get_current_user()->ID );
			...
			$setup_intent = WC_Criptanpay_API::request(
				[
			...
			if ( $setup_intent->error ) {
				...
				WC_Criptanpay_Logger::log("Failed create Setup Intent while saving a card.");
				WC_Criptanpay_Logger::log("Response: $error_response_message");
				throw new Exception( __( 'Your card could not be set up for future usage.', 'woocommerce-gateway-criptanpay' ) );
			}
			...
			if ( 'requires_action' === $setup_intent->status ) {
				...
				$response = [
					...
					'error'  => [
						...
						'message' => __( 'Failed to save payment method.', 'woocommerce-gateway-criptanpay' ),
					],
				];
			} else {
...
new WC_Criptanpay_Intent_Controller();
