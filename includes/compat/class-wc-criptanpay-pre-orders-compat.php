 ...
 *
 */
class WC_Criptanpay_Pre_Orders_Compat extends WC_Criptanpay_Payment_Gateway {
	public $saved_cards;

	public function __construct() {
		$this->saved_cards = WC_Criptanpay_Helper::get_settings( 'criptanpay', 'saved_cards' );
	}
	...
	public function remove_order_source_before_retry( $order ) {
		$order->delete_meta_data( '_criptanpay_source_id' );
		$order->delete_meta_data( '_criptanpay_card_id' );
		$order->save();
	}
	...
	public function process_pre_order( $order_id ) {
		try {
			...
			if ( empty( $prepared_source->customer ) || empty( $prepared_source->source ) ) {
				throw new WC_Criptanpay_Exception( __( 'Unable to store payment details. Please try again.', 'woocommerce-gateway-criptanpay' ) );
			}
			...
			WC_Criptanpay_Logger::log( 'Pre Orders Error: ' . $e->getMessage() );
	...
	public function process_pre_order_release_payment( $order, $retry = true ) {
		try {
			...
			if ( ! empty( $response->error ) && ! $is_authentication_required ) {
				...
				$order->update_status( 'failed', sprintf( __( 'Criptanpay charge awaiting authentication by user: %s.', 'woocommerce-gateway-criptanpay' ), $id ) );
				...
				do_action( 'wc_gateway_criptanpay_process_payment_authentication_required', $order );
			...
			$order_note = sprintf( __( 'Criptanpay Transaction Failed (%s)', 'woocommerce-gateway-criptanpay' ), $error_message );
