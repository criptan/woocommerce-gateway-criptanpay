...
/**
 * WC_Criptanpay_Customer class.
 *
 * Represents a Criptanpay Customer.
 */
class WC_Criptanpay_Customer {

	/**
	 * Criptanpay customer ID
	 * @var string
	 */
	private $id = '';
	...
	/**
	 * Store data from the Criptanpay API about this customer
	 */
	...
	protected function generate_customer_request( $args = array() ) {
		...
		if ( $user ) {
			...
			$description = sprintf( __( 'Name: %1$s %2$s, Username: %s', 'woocommerce-gateway-criptanpay' ), $billing_first_name, $billing_last_name, $user->user_login );
			...
			$description = sprintf( __( 'Name: %1$s %2$s, Guest', 'woocommerce-gateway-criptanpay' ), $billing_first_name, $billing_last_name );
		...
		$metadata             = array();
		$defaults['metadata'] = apply_filters( 'wc_criptanpay_customer_metadata', $metadata, $user );
	...
	public function create_customer( $args = array() ) {
		...
		$response = WC_Criptanpay_API::request( apply_filters( 'wc_criptanpay_create_customer_args', $args ), 'customers' );
		...
		do_action( 'woocommerce_criptanpay_add_customer', $args, $response );

		return $response->id;
	}

	/**
	 * Updates the Criptanpay customer through the API.
	 *
	...
	public function update_customer( $args = array(), $is_retry = false ) {
		if ( empty( $this->get_id() ) ) {
			throw new WC_Criptanpay_Exception( 'id_required_to_update_user', __( 'Attempting to update a Criptanpay customer without a customer ID.', 'woocommerce-gateway-criptanpay' ) );
		}

		$args     = $this->generate_customer_request( $args );
		$args     = apply_filters( 'wc_criptanpay_update_customer_args', $args );
		$response = WC_Criptanpay_API::request( $args, 'customers/' . $this->get_id() );

		if ( ! empty( $response->error ) ) {
			if ( $this->is_no_such_customer_error( $response->error ) && ! $is_retry ) {
				// This can happen when switching the main Criptanpay account or importing users from another site.
		...
		do_action( 'woocommerce_criptanpay_update_customer', $args, $response );
	...
	/**
	 * Add a source for this criptanpay customer.
	...
	public function add_source( $source_id ) {
			...
			if ( ! empty( $response->type ) ) {
				switch ( $response->type ) {
					...
					case 'sepa_debit':
						...
						$wc_token->set_gateway_id( 'criptanpay_sepa' );
					...
					default:
						if ( 'source' === $response->object && 'card' === $response->type ) {
							...
							$wc_token->set_token( $response->id );
							$wc_token->set_gateway_id( 'criptanpay' );
		...
		do_action( 'woocommerce_criptanpay_add_source', $this->get_id(), $wc_token, $response, $source_id );

		return $response->id;
	}

	/**
	 * Attaches a source to the Criptanpay customer.
	 *
	...
	public function attach_source( $source_id ) {
		...
		$response = WC_Criptanpay_API::request(
			array(
				'source' => $source_id,
			),
		...
		if ( ! empty( $response->error ) ) {
			// It is possible the WC user once was linked to a customer on Criptanpay
			...
			if ( $this->is_no_such_customer_error( $response->error ) ) {
				...
				return WC_Criptanpay_API::request( array(), 'sources/' . $source_id, 'GET' );
			} else {
				return $response;
			}
		} elseif ( empty( $response->id ) ) {
			return new WP_Error( 'error', __( 'Unable to add payment source.', 'woocommerce-gateway-criptanpay' ) );
		} else {
			return $response;
		}
	}
	...
	/**
	 * Get a customers saved sources using their Criptanpay ID.
	 *
	...
	public function get_sources() {
		...
		$sources = get_transient( 'criptanpay_sources_' . $this->get_id() );
		...
		if ( false === $sources ) {
			...
			set_transient( 'criptanpay_sources_' . $this->get_id(), $sources, DAY_IN_SECONDS );
		}
	...
	/**
	 * Delete a source from criptanpay.
	 * @param string $source_id
	 */
	public function delete_source( $source_id ) {
		...
		$response = WC_Criptanpay_API::request( array(), 'customers/' . $this->get_id() . '/sources/' . sanitize_text_field( $source_id ), 'DELETE' );

		$this->clear_cache();

		if ( empty( $response->error ) ) {
			do_action( 'wc_criptanpay_delete_source', $this->get_id(), $response );
	...
	/**
	 * Set default source in Criptanpay
	 * @param string $source_id
	 */
	public function set_default_source( $source_id ) {
		...
		if ( empty( $response->error ) ) {
			do_action( 'wc_criptanpay_set_default_source', $this->get_id(), $response );
	...
	public function clear_cache() {
		delete_transient( 'criptanpay_sources_' . $this->get_id() );
		delete_transient( 'criptanpay_customer_' . $this->get_id() );
		$this->customer_data = array();
	}

	/**
	 * Retrieves the Criptanpay Customer ID from the user meta.
	 *
	 * @param  int $user_id The ID of the WordPress user.
	 * @return string|bool  Either the Criptanpay ID or false.
	 */
	public function get_id_from_meta( $user_id ) {
		return get_user_option( '_criptanpay_customer_id', $user_id );
	}

	/**
	 * Updates the current user with the right Criptanpay ID in the meta table.
	 *
	 * @param string $id The Criptanpay customer ID.
	 */
	public function update_id_in_meta( $id ) {
		update_user_option( $this->get_user_id(), '_criptanpay_customer_id', $id, false );
	}
	...
	public function delete_id_from_meta() {
		delete_user_option( $this->get_user_id(), '_criptanpay_customer_id', false );
	}
